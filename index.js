...
const ROLE_TIER_DATA = [
    ...
    { roleId: '1339056251090243654', vouches: 5 }, // Changed from 15 to 5
...
async function processMentionsInChannel(searchChannel, guild, targetUserId = null, commandChannel = null) {
    ...
    try {
        ...
        try {
            ...
            await delay(500); // Reduced delay for better performance
        } catch (error) {
        ...
        while (totalMessages < maxMessages) {
            const fetchOptions = { limit: 50 }; // Reduced from 100 to 50 for better performance
...
client.on('messageCreate', async message => {
    ...
    console.log(`Received message from ${message.author.tag}: ${message.content} in channel ${message.channel.id}`);

    if (message.author.bot || !message.content.startsWith(PREFIX)) return;

    const args = message.content.slice(PREFIX.length).trim().split(/ +/);
    let command = args.shift()?.toLowerCase();
    console.log(`Received command: ${command} with args: ${args.join(', ')} from ${message.author.tag} in channel ${message.channel.id}`);
    ...
    if (command === 'vouch' && args[0]?.toLowerCase() === 'search') {
        ...
        try {
            ...
            await message.channel.send('Processing /vouch search command... This may take a moment.');

            const guild = client.guilds.cache.get(GUILD_ID);
            if (!guild) throw new Error(`Guild ${GUILD_ID} not found. Ensure the bot is in the correct server.`);

            console.log(`Processing /vouch search command with args: ${args.join(', ')}`);
