...
// Utility function for delays to prevent rate limiting
...
async function processMentionsInChannel(searchChannel, guild, targetUserId = null, commandChannel = null) {
    ...
    try {
        ...
        while (totalMessages < maxMessages) {
            ...
            await delay(1000); // Fix 1: Rate limit handling
        }
        ...
        await delay(500); // Fix 1: Rate limit handling
        ...
        if (targetUserId) {
            const member = guild.members.cache.get(targetUserId) || await guild.members.fetch(targetUserId).catch(error => {
                logError(`Failed to fetch member ${targetUserId}: ${error.message}`);
                return null;
            });
            if (member) {
                ...
                if (highestThreshold) {
                    ...
                    if (!member.roles.cache.has(highestThreshold.roleId)) {
                        ...
                        await outputChannel.send(`<@${member.user.id}> now has ${count} vouch${count === 1 ? '' : 'es'} and earned the <@&${highestThreshold.roleId}> role!`); // Fix 4: Use mention
                        await delay(500); // Fix 1: Rate limit handling
                    } else {
                        await outputChannel.send(`<@${member.user.id}> now has ${count} vouch${count === 1 ? '' : 'es'}.`); // Fix 4: Use mention
                        await delay(500); // Fix 1: Rate limit handling
                    }
                } else {
            ...
            // Batch role updates for all members
            const roleUpdates = [];
            for (const [userId, count] of vouchCounts) {
                ...
                if (member) {
                    ...
                    if (highestThreshold) {
                        for (const oldRoleId of currentRoles) {
                            if (oldRoleId !== highestThreshold.roleId && member.roles.cache.has(oldRoleId)) {
                                roleUpdates.push({ member, roleId: oldRoleId, action: 'remove' });
                            }
                        }
                        if (!member.roles.cache.has(highestThreshold.roleId)) {
                            roleUpdates.push({ member, roleId: highestThreshold.roleId, action: 'add', count });
                        }
                    } else {
                        for (const role of ROLE_TIER_DATA) {
                            if (member.roles.cache.has(role.roleId)) {
                                roleUpdates.push({ member, roleId: role.roleId, action: 'remove' });
                            }
                        }
                    }
                }
            ...
            // Process role updates in batches to avoid rate limits
            for (let i = 0; i < roleUpdates.length; i++) {
                const update = roleUpdates[i];
                try {
                    if (update.action === 'add') {
                        await update.member.roles.add(update.roleId).catch(error => logError(`Failed to add role ${update.roleId} to ${update.member.user.tag}: ${error.message}`));
                        await outputChannel.send(`<@${update.member.user.id}> now has ${update.count} vouch${update.count === 1 ? '' : 'es'} and earned the <@&${update.roleId}> role!`); // Fix 4: Use mention
                    } else if (update.action === 'remove') {
                        await update.member.roles.remove(update.roleId).catch(error => logError(`Failed to remove role ${update.roleId} from ${update.member.user.tag}: ${error.message}`));
                    }
                    await delay(500); // Fix 1: Rate limit handling
                } catch (error) {
                    logError(`Error processing role update for ${update.member.user.tag}: ${error.message}`);
                }
            }
        }
...
client.on('messageCreate', async message => {
    ...
    if (command === 'vouch' && args[0]?.toLowerCase() === 'search') {
        ...
        try {
            ...
            await message.channel.send(`Vouch counts and roles for <@${targetMember.user.id}> have been updated based on explicit @mentions in ${searchChannel.name}. Check <#${OUTPUT_CHANNEL_ID}> for details.`);
        } catch (error) {
...
client.on('shardDisconnect', (event, id) => {
    logError(`Shard ${id} disconnected: ${event.code} - ${event.reason}`);
...
client.on('shardResume', (id, replayedEvents) => {
    console.log(`Shard ${id} resumed successfully, replayed ${replayedEvents} events.`);
});
