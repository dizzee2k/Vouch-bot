...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'vouchsearch') {
        ...
        try {
            ...
            await processMentionsInChannel(searchChannel, guild, targetUserId, interaction.channel);
            const targetMember = guild.members.cache.get(targetUserId);
            await interaction.editReply(`Vouch counts and roles for ${targetMember.user.tag} have been updated based on explicit @mentions in ${searchChannel.name}. Check <#${OUTPUT_CHANNEL_ID}> for details.`);
            console.log(`Successfully processed slash command vouchsearch for ${targetMember.user.tag}`);
        } catch (error) {
