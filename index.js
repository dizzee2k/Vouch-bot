...
// Helper function to fetch the output channel
async function getOutputChannel(guild) {
    try {
        const outputChannel = await guild.channels.fetch(OUTPUT_CHANNEL_ID);
        if (!outputChannel || !outputChannel.isTextBased()) {
            throw new Error(`Output channel ${OUTPUT_CHANNEL_ID} is not a text channel.`);
        }

        // Check permissions in the output channel
        ...
        if (missingPermissions.length > 0) {
            ...
            throw new Error(`Bot lacks the following permissions in output channel ${outputChannel.name}: ${missingPermNames.join(', ')}.`);
        }
...
async function processMentionsInChannel(searchChannel, guild, targetUserId = null, commandChannel = null) {
    let outputChannel;
    try {
        // Fix 1: Fetch the output channel for sending messages
        outputChannel = await getOutputChannel(guild);
    } catch (error) {
        logError(`Cannot proceed without output channel: ${error.message}`);
        if (commandChannel) {
            await commandChannel.send(`Failed to send output to #vouch-check-command-center: ${error.message}. Please ensure the bot has access to that channel.`);
        }
        return;
    }
    ...
    try {
        ...
        if (missingPermissions.length > 0) {
            ...
            throw new Error(`Bot lacks the following permissions in search channel ${searchChannel.name}: ${missingPermNames.join(', ')}. Please grant these permissions to the bot.`);
        }
        ...
        // Fix 1: Send output to the output channel
        ...
        if (commandChannel) {
            await commandChannel.send(`An error occurred while processing vouches. Check the output in <#${OUTPUT_CHANNEL_ID}> for details.`);
        }
    }
}
...
client.on('messageCreate', async message => {
    ...
    if (command === 'vouch' && args[0]?.toLowerCase() === 'search') {
        ...
        try {
            ...
            // Fix 2: Send command response to the original channel
